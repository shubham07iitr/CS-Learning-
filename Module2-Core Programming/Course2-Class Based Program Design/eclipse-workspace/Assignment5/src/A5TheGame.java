...
class MyWorldFinal extends World{
	...
	public MyWorldFinal onTick() {
		if (this.noTicks%10 == 0) {this.noTicks = this.noTicks + 1; return this.addSS().moveLob().moveLoss().removeBullets().removeSS().processCollission().updateScore();}
		else {this.noTicks = this.noTicks + 1; return this.moveLob().moveLoss().removeBullets().removeSS().processCollission().updateScore();}}
...
//METHOD MOVELOB
	...
	//And we will reduce bullet count by 1
	//Signature Self > IList<SpaceShip>
	public IList<Bullet> removeBulletsHelper(IList<Bullet> lob) {
		if (lob.isEmpty()) {return lob;}
		else {if (lob.getFirst().pos.x >= 500 || lob.getFirst().pos.x <= 0 || lob.getFirst().pos.y >= 500 || lob.getFirst().pos.y <= 0) {this.bulletsRemaining = this.bulletsRemaining - 1; return this.removeBulletsHelper(lob.getRest());}
				 else {return new ConsList<Bullet>(lob.getFirst(), this.removeBulletsHelper(lob.getRest()));}}}
	
	
	//METHOD PROCESSCOLLISSION
	//First get a list of all spaceships and bullets which are in a collission
	//Remove those ss and bullets from the list of bullets and list of ss and mutate the original lists
	//return the current myworld
	//Signature > Self > MyWorldFinal
	
	public MyWorldFinal processCollission() {
		IList<Bullet> hitBulletsList = this.processCollissionHelperBullets(this.lob, this.loss);
		IList<SpaceShip> hitSSList = this.processCollissionHelperSS(this.lob, this.loss);
		this.bulletsRemaining = this.bulletsRemaining - hitBulletsList.lenList();
		this.ssDestroyed = this.ssDestroyed + hitSSList.lenList();
		this.lob = this.lob.removeSubList(hitBulletsList);
		this.loss = this.loss.removeSubList(hitSSList); 
		return this;
	}
	
	//METHOD PROCESSCOLLISSIONHELPERBULLETS
	//Returns a sublist of all the bullets which have been hit at the given tick
	//Signature> Self, IList<Bullets>, IList<SpaceShip> > IList<Bullets>
	
	public IList<Bullet> processCollissionHelperBullets(IList<Bullet> lob, IList<SpaceShip> loss){
		if (lob.isEmpty()) {return lob;}
		else { if (lob.getFirst().hasHit(loss)) {return new ConsList<Bullet> (lob.getFirst(), this.processCollissionHelperBullets(lob.getRest(), loss));}
					 else {return this.processCollissionHelperBullets(lob.getRest(), loss);} } }
	
//METHOD PROCESSCOLLISSIONHELPERSS
	//Returns a sublist of all the SS which have been hit at the given tick
	//Signature> Self, IList<Bullets>, IList<SpaceShip> > IList<SS>
	
	public IList<SpaceShip> processCollissionHelperSS(IList<Bullet> lob, IList<SpaceShip> loss){
		if (loss.isEmpty()) {return loss;}
		else { if (loss.getFirst().isHit(lob)) {return new ConsList<SpaceShip> (loss.getFirst(), this.processCollissionHelperSS(lob, loss.getRest()));}
					 else {return this.processCollissionHelperSS(lob, loss.getRest());} } }
