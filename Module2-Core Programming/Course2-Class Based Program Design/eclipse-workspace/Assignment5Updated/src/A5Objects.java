...
class Bullet {
	...
	Bullet (int radius, Posn pos) {
		...
		color = Color.GREEN;
		this.pos= pos;
		this.velocity = 30; //Defined as default
...
class Missile {
	...
	Missile (Posn pos) {
		this.shape = new AboveImage(new RectangleImage(20, 20, OutlineMode.SOLID, Color.RED), 
...
class SpaceShip {
	...
	SpaceShip  (Posn pos, int direction) {
		this.shape = new RectangleImage(40, 20, OutlineMode.SOLID, Color.BLUE);
		this.pos = pos; 
...
class ScoreBoard {
	...
	ScoreBoard(int bulletsLeft, int spaceshipsDestroyed) {
		this.bulletsLeft = bulletsLeft; 
		this.spacesihpsDestroyed	= spaceshipsDestroyed;
		this.shape = new AboveImage (new TextImage("Bullets Left" + ":" +  " " + Integer.toString(this.bulletsLeft), 20, Color.BLACK),
				new TextImage("SS Destroyed" + ":" + " " + Integer.toString(this.spacesihpsDestroyed), 20 , Color.BLACK));
	}
}
