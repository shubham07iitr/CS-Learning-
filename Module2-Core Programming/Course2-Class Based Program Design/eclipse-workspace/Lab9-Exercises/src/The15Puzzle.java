...
class FifteenGame extends World {
  ...
  public WorldScene makeScene() {
  	...
  	for (int i = 0; i <= 3; i ++) {
  		for (int j = 0; j <= 3; j++) {
  			ws.placeImageXY(this.tiles.get(i).get(j).drawAt(i, j), j*100+50, i*100+50);
  		}}
  	return ws; }
  ...
  public void onKeyEvent(String k) {
    ...
    // extra: handle "u" to undo moves
  	...
  	if (k.equals("up") && this.upPossible()) {util.swap(this.tiles, indexOf0, k); this.indexOf0 = this.indexOf0 - 4; keysPressed.add(k);}
  		...
  		if (lastMove.equals( "up")) {util.swap(this.tiles, indexOf0, "down"); this.indexOf0 = this.indexOf0 + 4; keysPressed.add("down");}
  		else if (lastMove.equals("down")) {util.swap(this.tiles, indexOf0, "up"); this.indexOf0 = this.indexOf0 - 4; keysPressed.add("up");}
  		else if (lastMove .equals("left")) {util.swap(this.tiles, indexOf0, "right"); this.indexOf0 = this.indexOf0 + 1; keysPressed.add("right");}
