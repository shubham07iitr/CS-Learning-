...
class MyGame extends World{
	...
	MyGame(int width, int height, int currentTick, int endTick, boolean randomText, boolean randomCircles){
		if ( width < 0 || height < 0 || endTick < 2) {
			throw new IllegalArgumentException("Invalid arguments passed to constructor.");
		}
		this.WIDTH = width;
	...
	WorldScene addInfoToScene(WorldScene scene) {
		return scene.placeImageXY( new TextImage("Final tick: " + Integer.toString(this.finalTick) + "  Current tick: " + Integer.toString(this.currentTick) , Color.black),100, 20);
	}
	...
	public MyGame addRandomText() {
		return new MyGame(this.WIDTH, this.HEIGHT, this.currentTick, this.finalTick, true, this.randomCircles);
	}
	
	public MyGame addRandomCircles() {
		return new MyGame(this.WIDTH, this.HEIGHT, this.currentTick, this.finalTick, this.randomText, true);
	}
	
	public MyGame incrementGameTick() {
		return new MyGame(this.WIDTH, this.HEIGHT, this.currentTick + 1, this.finalTick,this.randomText, this.randomCircles);
	}
	
	public MyGame onKeyEvent(String key) {
		 //did we press the space update the final tick of the game by 10. 
		 if (key.equals(" ") ) {
			 return new MyGame(this.WIDTH, this.HEIGHT, this.currentTick, this.finalTick+10, true, false);
		 }else {
			 return this;
		 }
	}
