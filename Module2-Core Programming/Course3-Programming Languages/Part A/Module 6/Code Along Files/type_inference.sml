(*
 f :  T1 -> T2 [must be a functino; all functions take 1 argument ]
 x : T1
 y : T3
 z : T4
 ...
 So T1 = int * int
 So (abs y) + z : int , so let exp: int, so body : int
 T2 = int
 f : int*int -> int
...
(* Another example *)
(* suming all the elements in a list *)
...
(*
 ...
 T1 = T3 List
 T2 = int [because 0 may be returned]
