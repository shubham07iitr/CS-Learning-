...
; took template from Cow
(define (render-cow c)
  (place-image (choose-image c) (cow-x c) CTR-Y MTS))  ; wishing for a new function choose-image which can figure out RCow or LCow

;; Cow -> Image
;; produce RCOW or LCOW depending on direcion cow is going
(check-expect (choose-image (make-cow 10 3)) RCOW)
(check-expect (choose-image (make-cow 10 4)) LCOW)
