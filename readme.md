# What is this 

This is my attempt at learning fundamentals of Computers Science degree (including ML) as taught in a bachelor's course + some additional applications of CS courses as a non CS grad. Most of the courses have been taken up as suggested by the [OSSU curriculum ](https://github.com/ossu/computer-science?tab=readme-ov-file#core-cs). Some other courses/resources cover the fundamentals as suggested by [teachyourselfcs curriculum](https://teachyourselfcs.com/) and some courses are just generic application courses such as ML or web development, or introductory courses


# Courses Listg
Organised as Completed, In Progress, Yet to Begin , also qualified as OSSU or Non OSSU, mentioned in roughly the order in which I completed these,

## Completed

- [CS50](https://cs50.harvard.edu/x/) (Non OSSU)
- [Introduction to Computer Science and Programming using Python](https://cs.ossu.dev/coursepages/intro-cs/) (OSSU)
- [CS50 Python](https://cs50.harvard.edu/python/) (Non OSSU)
- [Systematic Program Design](https://cs.ossu.dev/coursepages/spd/) (OSSU)
- [Database modeling and Theory](https://www.edx.org/learn/databases/stanford-university-databases-modeling-and-theory) (OSSU)
- [Databases: Relational DB and SQL](https://www.edx.org/learn/relational-databases/stanford-university-databases-relational-databases-and-sql) (OSSU)
- [Class-based Program Design](https://course.ccs.neu.edu/cs2510sp22/index.html) (OSSU)
- [Programming Languages A](https://www.coursera.org/learn/programming-languages) (OSSU)
- [Programming Languages B](https://www.coursera.org/learn/programming-languages-part-b) (OSSU)
- [Programming Languages C](https://www.coursera.org/learn/programming-languages-part-c) (OSSU)
- [CS50 Web Development](https://cs50.harvard.edu/web/weeks/8/) (Non OSSU)
- [Full Stack Web Dev](https://www.udemy.com/course/the-complete-web-development-bootcamp/) (Non OSSU)
- [Python Data Analyis and Visualisation](https://www.udemy.com/course/python-data-analysis-visualization/learn/lecture/29948152?start=0#overview) (Non OSSU)
- [Missing Sem of your CS Education](https://missing.csail.mit.edu/) (OSSU)

## In Progress

- [100 days of Code](https://www.udemy.com/course/100-days-of-code/) (Non OSSU)
- [Nand to Tetris - Part A](https://www.coursera.org/learn/build-a-computer) (OSSU)
- [Machine Learning Specialisation](https://www.coursera.org/learn/machine-learning) (OSSU)

## Yet to Begin

- [Object Oriented Design](https://course.ccs.neu.edu/cs3500f19/) (OSSU)
- [Software Architecture](https://www.coursera.org/learn/software-architecture) (OSSU)
- [Code: The Hidden Language of Computer Hardware and Software](https://bobcarp.wordpress.com/wp-content/uploads/2014/07/code-charles-petzold.pdf) (Non OSSU)
- [The Pragmatic Programmer](https://bobcarp.wordpress.com/wp-content/uploads/2014/07/code-charles-petzold.pdf) (Non OSSU)
- [The Philosophy of Software Design](https://web.stanford.edu/~ouster/cgi-bin/aposd2ndEdExtract.pdf) (Non OSSU)
- [Some Course on DevOps](Yet to be Decided) (Non OSSU)
- [Build a Modern Computer from First Principles: Nand to Tetris Part II](https://www.coursera.org/learn/nand2tetris2) (OSSU)
- [Computer Systems: A Programmers Perspective](https://www.cs.cmu.edu/afs/cs/academic/class/15213-f15/www/index.html) (Non OSSU)
- [Operating Systems: Three Easy Pieces](http://pages.cs.wisc.edu/~remzi/Classes/537/Spring2018/) (Non OSSU)
- [Intro to DB Systems](https://www.youtube.com/playlist?list=PLYp4IGUhNFmw8USiYMJvCUjZe79fvyYge) (Non OSSU)
- [Database Internals](http://103.203.175.90:81/fdScript/RootOfEBooks/E%20Book%20collection%20-%202025%20-%20D/CSE%20%20IT%20AIDS%20ML/Database_Internals_A_Deep_Dive_into_How_Distributed_Data_Systems.pdf) (Non OSSU)
- [Calculus 1A: Differentiation](https://openlearninglibrary.mit.edu/courses/course-v1:MITx+18.01.1x+2T2019/about) (OSSU)
- [Calculus 1B: Integration](https://openlearninglibrary.mit.edu/courses/course-v1:MITx+18.01.2x+3T2019/about) (OSSU)
- [Calculus 1C: Coordinate Systems & Infinite Series](https://openlearninglibrary.mit.edu/courses/course-v1:MITx+18.01.3x+1T2020/about) (OSSU)
- [Databases: Semistructured Data](https://www.edx.org/course/semistructured-data) (OSSU)
- [Software Debugging](https://www.youtube.com/playlist?list=PLAwxTw4SYaPkxK63TiT88oEe-AIBhr96A) (OSSU)
- [Software Testing](https://www.youtube.com/playlist?list=PLAwxTw4SYaPkWVHeC_8aSIbSxE_NXI76g) (OSSU)
- [Essence of Linear Algeba](https://www.youtube.com/playlist?list=PLZHQObOWTQDPD3MizzM2xVFitgF8hE_ab) (OSSU)
- [Probability](https://projects.iq.harvard.edu/stat110/home) (OSSU)
- [Linear Algebra](https://ocw.mit.edu/courses/mathematics/18-06sc-linear-algebra-fall-2011/) (OSSU)
- [Mathematics for Computer Science](https://openlearninglibrary.mit.edu/courses/course-v1:OCW+6.042J+2T2019/about) (OSSU)
- [Divide and Conquer, Sorting and Searching, and Randomized Algorithms](https://www.coursera.org/learn/algorithms-divide-conquer) (OSSU)
- [Graph Search, Shortest Paths, and Data Structures](https://www.coursera.org/learn/algorithms-graphs-data-structures) (OSSU) 
- [Greedy Algorithms, Minimum Spanning Trees, and Dynamic Programming](https://www.coursera.org/learn/algorithms-greedy) (OSSU)
- [Shortest Paths Revisited, NP-Complete Problems and What To Do About Them](https://www.coursera.org/learn/algorithms-npcomplete) (OSSU)
- [Software Engineering: Introduction](https://www.edx.org/learn/computer-science/stanford-university-compilers) (OSSU)
- [Parallel Programming](https://www.coursera.org/learn/scala-parallel-programming) (OSSU)
- [Compilers](https://www.edx.org/learn/computer-science/stanford-university-compilers) (OSSU)
- [Theory of Computation](https://ocw.mit.edu/courses/18-404j-theory-of-computation-fall-2020/) (OSSU)
- [Crafting Interpreters](https://github.com/exoad/booksbox/blob/main/Crafting%20Interpreters%20Robert%20Nystrom.pdf) (Non OSSU)

## ML Specific Courses (Part of Module6 - Core Applications)
- [Machine Learning Specialisation](https://www.coursera.org/learn/machine-learning) (OSSU) (In Progress)
- [Introduction to Statistical Learning](https://www.stat.berkeley.edu/~rabbee/s154/ISLR_First_Printing.pdf) (Non OSSU) (Yet to begin)
- [Elements of Statistical Learning](https://www.sas.upenn.edu/~fdiebold/NoHesitations/BookAdvanced.pdf) (Non OSSU) (Yet to begin)
- [Pattern Recognition and Machine Learning](https://www.microsoft.com/en-us/research/wp-content/uploads/2006/01/Bishop-Pattern-Recognition-and-Machine-Learning-2006.pdf) (Non OSSU) (Yet to begin)
- [Deep Learning Specialisation](https://www.coursera.org/specializations/deep-learning) (OSSU) (Yet to begin)
- [Deep Learning by GoodFellow](https://aikosh.indiaai.gov.in/static/Deep+Learning+Ian+Goodfellow.pdf) (Non OSSU) (Yet to begin)
- [Designing Machine Learning Systems](http://103.203.175.90:81/fdScript/RootOfEBooks/E%20Book%20collection%20-%202025%20-%20H/AI%20and%20DS/designing.machine.learning.systems.pdf) (Non OSSU) (Yet to begin)
- [Machine Learning Engineering](https://soclibrary.futa.edu.ng/books/Machine%20Learning%20Engineering%20(Andriy%20Burkov)%20(Z-Library).pdf) (Non OSSU) (Yet to begin)


# Organisation

## Folder Structure

Computer Science
├── Module1-Intro to CS
├── Module2-Core Programming
├── Module3-Core Maths
├── Module4-Core Systems
├── Module5-Core Theory
├── Module6-Core Applications
├── Module7-Advanced Courses
├── Module8-Advanced Courses
├── Module9-ML
└── readme.md


## Otherp points on structure
- Curicullum is organised into modules and each module will have:
    - OSSU Courses
    - Non OSSU courses (if applicable)
- Each course folder would have roughly 3-4 folders and files:
    - Course Notes as provided by the instructor
    - Course Notes taken by me (more detailed and will cover a lot of random topics which intrigued me during the course) - generally at the top of the folder
    - Exercises of the course (Most of it completed)
    - Lecture Codes (For coding discussion during lectures)

## Module to Course mapping

| Module | OSSU Courses | Non-OSSU Courses |
|------|-------------|----------------|
| **Module1 – Intro to CS** | Introduction to Programming – MIT, Missing Semester of CS Education | CS50 – Intro to Computer Science, 100 Days of Python, CS50 – Intro to Python, Data Analysis & Visualisation in Python |
| **Module2 – Core Programming** | Systematic Program Design, Class-Based Program Design, Programming Languages, Object-Oriented Design, Software Architecture | Pragmatic Programmer, Philosophy of Software Design |
| **Module3 – Core Maths** | Calculus, Linear Algebra, Probability, Mathematics for Computer Science | Essence of Linear Algebra |
| **Module4 – Core Systems** | Nand to Tetris, Operating Systems: Three Easy Pieces | Amateur Systems Understanding, Code – The Hidden Language of Hardware & Software, Computer Systems: A Programmer’s Perspective (CSAPP), Udemy Computer Networking |
| **Module5 – Core Theory** | Algorithms I, Algorithms II, Algorithms III, Algorithms IV | — |
| **Module6 – Core Applications** | Databases: Theory & Modeling, Relational Databases & SQL, Semistructured Data | CS50W, Angela Yu Bootcamp, Intro to DB Systems, DevOps, Software Engineering, Database Internals, Frontend Practice |
| **Module7 – Advanced Courses** | Software Debugging, Software Testing, Compilers, Theory of Computation | Crafting Interpreters |
| **Module8 – Machine Learning** | Machine Learning Specialization, Deep Learning Specialization | ISL, ESL, PRML, Deep Learning (Goodfellow), Designing ML Systems, Machine Learning Engineering |

