...
void tabulate(void)
{
    for (int i= 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            if (preferences[i][j] == 1 && !candidates[j].eliminated)
            {
                candidates[j].votes++;
                break;
            }
...
int find_min(void)
{
    int min_count = candidates[0].votes;
    for (int i = 1; i < candidate_count; i++)
    {
        if (candidates[i].votes < min_count && candidates[i].eliminated == false)
        {
...
void eliminate(int min)
{
    for (int i = 0 ; i < candidate_count; i++)
    {
        if (candidates[i].votes == min)
        {
            ...
            {
                int eliminated_rank = preferences[j][i];
                if (eliminated_rank > 0)
                {
                    preferences[j][i] = 0;  // mark eliminated candidate as invalid
                    for (int k = 0; k < candidate_count; k++)
                    {
                        if (preferences[j][k] > eliminated_rank)
                        {
                            preferences[j][k]--;
                        }
                    }
