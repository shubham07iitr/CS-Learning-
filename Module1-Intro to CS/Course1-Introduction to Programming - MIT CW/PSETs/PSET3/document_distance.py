...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for key, value in final_dict.items():
        ...
        return_list.append(sum(value))
    
    final_return_list = []
    for key, value in final_dict.items():
        if value == max(return_list):
            final_return_list.append(key)
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    """
    working_list = text_to_list(load_file(file_path))
    working_dict = get_frequencies(working_list)
    sum_of_words = 0
    for key, value in working_dict.items():
        sum_of_words += value

    for key, value in working_dict.items():
        working_dict[key] = value/sum_of_words
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    working_list = []
    working_list_flattened = []
    for i in file_paths:
        working_list.append(text_to_list(load_file(i)))
        working_list_flattened += text_to_list(load_file(i))

    return_dict_temp = get_frequencies(working_list_flattened)
    for key, value in return_dict_temp.items():
        count = 0
        for i in working_list:
            if key in i:
                count += 1
        if count != 0:
            return_dict_temp[key] = math.log10(total_docs/count)
        else:
            return_dict_temp[key] = 0.0

    return return_dict_temp
    pass
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    for key, value in dict_tf.items():
        if key in dict_idf:
            return_list.append((key, dict_tf[key]*dict_idf[key]))
    return return_list
    pass
    pass
